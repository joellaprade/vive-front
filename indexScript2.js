...
var actualRank = Number(String(actualYear) 
        + String(actualMonth < 10 ? String(0) + String(actualMonth) : String(actualMonth))
        + String(actualDay < 10 ? String(0) + String(actualDay) : String(actualDay))
        )
...
const sort = (arr) => {
    ...
    do {
        ...
        for(let i = 0; i < arr.length - 1; i++){
            if(arr[i].fecha.rank > arr[i + 1].fecha.rank){
                let temp = arr[i];
                arr [i] = arr[i + 1];
                arr[i + 1] = temp;
                swapped = true;
...
const setDates = () => {
    ...
    for(let i = 0; i < eventosIngresados.length; i++) {
        if(eventosIngresados[i].fecha.rank >= actualRank){
            document.getElementById('upcomming-events').innerHTML +=
            
            `<a target="_blank" href="${eventosIngresados[i].linkEvento}" id="event-${count}">
                    <h4>
                        ${eventosIngresados[i].nombreEvento}
                    </h4>
                    <p>
                        ${eventosIngresados[i].fecha.mesInicial.nombre + " " + Number(eventosIngresados[i].fecha.diaInicial + 1) + " - "}
                        ${eventosIngresados[i].fecha.mesInicial.numero == eventosIngresados[i].fecha.mesFinal.numero ? eventosIngresados[i].fecha.diaFinal + 1 : eventosIngresados[i].fecha.mesFinal.nombre + " " + Number(eventosIngresados[i].fecha.diaFinal + 1)
                        }
                    </p>
                </a>`
            document.getElementById('upcomming-events-mobil').innerHTML +=
...
var elements = { 
    ...
    },
    {
        ...
        preview: "Pas&eacute; de ser un adolescente sin ganas de vivir encerrado en un cuarto a ya un adulto empezando a luchar por sus sueños con una sonrisa en la cara...",
        testimonial: "     Hola, mi nombre es Sebasti&aacute;n, tengo 19 años y quiero contar mi experiencia con los Entrenamientos Vive. Durante una gran parte mi vida en la adolescencia me sent&iacute;a perdido, triste, con miedo al futuro y en general sin saber que hacer conmigo hasta el punto de llegar a tener pensamientos suicidas y acciones de auto lesiones. Fue en esta misma &eacute;poca en donde conoc&iacute; el entrenamiento ya que me lo recomend&oacute; un familiar. Al principio ten&iacute;a muchas dudas al respecto ya que no sab&iacute;a qu&eacute; era y menos de lo que hac&iacute;a, sin embargo, me arm&eacute; de valent&iacute;a y realic&eacute; estos entramientos. Podr&iacute;a hablar años de lo mucho que me ayud&oacute;, de forma resumida dir&eacute; que el yo de hace unos dos años estar&iacute;a impresionado de la persona que soy al d&iacute;a de hoy, una persona segura, determinada, apasionada y sobre todo feliz. Pas&eacute; de ser un adolescente sin ganas de vivir encerrado en un cuarto a ya un adulto empezando a luchar por sus sueños con una sonrisa en la cara.",
        id: 3
    },
    {
        name: "Carlos (Charlie)",
    ...
    },
    {
        ...
        preview: "Durante mi proceso me he encontrado a m&iacute; misma, mis pasiones, conectado con mi poder femenino interior, despertado dones y talentos que hab&iacute;a mantenidos dormidos por mucho tiempo, me empodere de mi vida, deje de buscar culpables y me reconoc&iacute; dueña de mi vida...",
        testimonial: "     Como profesional en mi vida cre&iacute;a que todo giraba en torno al &eacute;xito en los negocios o en alcanzar un mejor puesto o salario, y por mucho tiempo hab&iacute;a dejado de lado mis sueños. Viv&iacute;a exigi&eacute;ndome, siendo una jueza dura conmigo misma y con mi familia. En mi interior sab&iacute;a que hab&iacute;a otro camino, y por eso no dude en elegir inscribirme y completar el entrenamiento transformacional VIVE. Durante mi proceso me he encontrado a m&iacute; misma, mis pasiones, conectado con mi poder femenino interior, despertado dones y talentos que hab&iacute;a mantenidos dormidos por mucho tiempo, me empodere de mi vida, deje de buscar culpables y me reconoc&iacute; dueña de mi vida. Las herramientas que he aprendido en cada paso Intro, Avanzado y Vive, me abrieron la posibilidad de observarme, creer y confiar en m&iacute;, llenarme de amor, para dar amor a otros, sanar  heridas, para sanar relaciones con las personas que amo y son importantes en mi vida. Hoy puedo decir que la abundancia, la plenitud y la prosperidad existen en mi vida, porque soy capaz de crearlas.",
        id: 6
    },
    {
...
for(i = 0; i < elementNumber; i++){
    container.innerHTML += 
        `<div id="${ i }" class=\"testimonial \" >
            ...
            <p>${ elements.testimonials[i].preview }</p>
            <a href="../testimonios/testimonios.html">
                <button>Leer testimonios</button>
            </a>
        </div>`;
}
...
boxes[middleElement].style.transition = "transform 0s ease-in-out, opacity 0.5s ease-in";
...
const fadersHalf = document.querySelectorAll('.fade-in-half')

const appearOptionsHalf = {
    threshold: 0.5,
}

const appearOnScrollHalf = new IntersectionObserver
(function(
    entries,
    appearOnScrollHalf
) {
    entries.forEach(entry => {
        if(!entry.isIntersecting){
            ...
            entry.target.classList.add('appear-half')
            appearOnScrollHalf.unobserve(entry.target)
        }
    })
},
appearOptionsHalf)

fadersHalf.forEach(fader => {
    appearOnScrollHalf.observe(fader);
});